...
train_path = "vae_train_data.npy"
validate_path = "vae_validate_data.npy"
test_path = "vae_test_data.npy"
...
def create_mask(data):
    ...
    data_gd = []
    for x in data:
        temp_gd = np.concatenate((x[0],x[1]),axis=1)
        temp_x = np.concatenate((x[0],x[1]),axis = 1)
        temp_x[4:,::] = 0
...
model = MaskNN(input_dims = vector_num ,
        hidden_dims = vector_num + 128,output_dims = vector_num,time_steps = total_len)
...
for epoch in range(total_epoch):
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.cpu().state_dict(), "backup/model_v1_" + str(epoch) + ".pt")
        model.cuda()